...
- `api.ts` — `apiFetch(path, init?: RequestInit & { auth?: boolean; silent?: boolean })` → `Response`.
  Adds base URL + `/api` prefix + Bearer header; pass `/todos` or `todos`, NOT
  `/api/todos`. Non-OK responses trigger the shared error toast unless an internal,
  best-effort request explicitly passes `silent: true`; still check `response.ok`. Also exports
